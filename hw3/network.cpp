...
void Network::addPost(int ownerId, std::string message, int likes, bool isIncoming, std::string authorName, bool isPublic){
    ...
    Post* p;
    if (isIncoming){
        p = new IncomingPost(nextPostId++, ownerId, message, likes, isPublic, authorName);
    }else{
        p = new Post(nextPostId++, ownerId, message, likes);
    }
    owner->addPost(p);
}
...
std::string Network::getPostsString(int ownerId, int howMany, bool showOnlyPublic){
    User* owner = getUser(ownerId);
    if (owner == nullptr) return "";
    return owner->getPostsString(howMany, showOnlyPublic);
}
...
int Network::readPosts(char* fname) {
    ...
    std::istringstream(line) >> postCount;

    for (int i = 0; i < postCount; ++i) {
        int messageId, ownerId, likes;
        std::string message, visibility, author;
...
int Network::writePosts(char* fname){
    ...
    for (auto post : posts){
        ...
        file << "\t" << post->getLikes() << "\n";
        if (post->getAuthor() != ""){
            file << "\t" << (post->getIsPublic() ? "public" : "private" ) << "\n";
            file << "\t" << post->getAuthor() << "\n";
        }else{
            file << "\t" << "\n";
            file << "\t" << "\n";
